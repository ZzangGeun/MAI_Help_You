...
class ChatbotAskAPIView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: ChatbotAskResponseSerializer},
        tags=["Chatbot"]
    )
    def post(self, request):
        ...
        try:
            ...
            return Response({'error': f'답변 생성 오류: {e}', 'status': 'error', 'user_id': user_id}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ChatbotHistoryAPIView(APIView):
    ...
    @extend_schema(
        summary="채팅 히스토리 조회",
        responses={200: ChatbotHistoryResponseSerializer},
        tags=["Chatbot"]
    )
    def get(self, request):
        try:
            ...
            return Response({'history': history, 'status': 'success', 'user_id': user_id}, status=status.HTTP_200_OK)
        except Exception as e:
            logger.error(f"히스토리 조회 오류: {e}")
            return Response({'error': f"히스토리 조회 중 오류: {str(e)}", 'status': 'error'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ChatbotClearHistoryAPIView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: ChatbotClearHistoryResponseSerializer},
        tags=["Chatbot"]
    )
    def post(self, request):
        try:
            ...
            if success:
                return Response({'message': "채팅 히스토리가 초기화되었습니다.", 'status': 'success', 'user_id': user_id}, status=status.HTTP_200_OK)
            else:
                return Response({'error': "히스토리 초기화에 실패했습니다.", 'status': 'error'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
            logger.error(f"히스토리 초기화 오류: {e}")
            return Response({'error': f"히스토리 초기화 중 오류: {str(e)}", 'status': 'error'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ChatbotHealthCheckAPIView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: ChatbotHealthCheckResponseSerializer},
        tags=["Chatbot"]
    )
    def get(self, request):
        try:
            ...
            if test_response.get('response'):
                return Response({
                    ...
                    'details': {'service': 'chatbot', 'test_response': 'OK'}
                }, status=status.HTTP_200_OK)
            else:
                return Response({
                    'status': 'unhealthy',
                    'timestamp': timezone.now(),
                    'details': {'service': 'chatbot', 'error': 'No response from service'}
            ...
            return Response({
                'status': 'unhealthy',
                'timestamp': timezone.now(),
                'details': {'service': 'chatbot', 'error': str(e)}
            }, status=status.HTTP_503_SERVICE_UNAVAILABLE)
